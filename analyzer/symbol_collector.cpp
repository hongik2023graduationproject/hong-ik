    ...
    std::string functionSignature(const FunctionStatement& fn) {
        ...
        for (size_t i = 0; i < fn.parameters.size(); ++i) {
            ...
            if (i < fn.parameterTypes.size() && fn.parameterTypes[i]) {
                ...
                if (i < fn.parameterOptionals.size() && fn.parameterOptionals[i]) {
                    s += "?";
                }
                s += " ";
            }
...
void SymbolCollector::addFunction(const std::shared_ptr<FunctionStatement>& fn, const std::string& container,
    ...
    for (size_t i = 0; i < fn->parameters.size(); ++i) {
        ...
        std::string typeText =
            (i < fn->parameterTypes.size() && fn->parameterTypes[i]) ? fn->parameterTypes[i]->text : "";
        if (i < fn->parameterOptionals.size() && fn->parameterOptionals[i]) {
            typeText += "?";
        }
...
void SymbolCollector::walkStatement(
    ...
    if (auto init = std::dynamic_pointer_cast<InitializationStatement>(stmt)) {
        ...
        out.push_back(
            makeInfo(cls->name, DocSymbolKind::Class, "클래스 " + cls->name, container, cls->nameToken, cls->line));
        const long long classIdx = static_cast<long long>(out.size()) - 1;
        for (size_t i = 0; i < cls->fieldNames.size(); ++i) {
            ...
            out.back().ownerClassIndex = classIdx;
        }
        for (size_t i = 0; i < cls->constructorParams.size(); ++i) {
            const auto& p        = cls->constructorParams[i];
            std::string typeText = (i < cls->constructorParamTypes.size() && cls->constructorParamTypes[i])
                                     ? cls->constructorParamTypes[i]->text
                                     : "";
        ...
        for (const auto& m : cls->methods) {
            ...
            addFunction(m, cls->name, DocSymbolKind::Method, out);
            out[before].ownerClassIndex = classIdx;
        }
    } else if (auto fe = std::dynamic_pointer_cast<ForEachStatement>(stmt)) {
        out.push_back(makeInfo(fe->elementName, DocSymbolKind::Variable, fe->elementType ? fe->elementType->text : "",
            container, fe->nameToken, fe->line));
        ...
        for (const auto& b : mt->caseBodies) {
            walkBlock(b, container, out);
        }
