...
- **plan B-1 Task 0에서 연산자×피연산자 타입 조합 전수 실측** (evaluator·VM 각각): `+ - * / % ** & | && || < > <= >= == !=` × {정수, 실수, 문자, 논리, 배열, 사전, 없음}. README 추정 금지, spec에 표로 박제.
- 두 런타임 모두 거부 → TC601. 한쪽만 거부 → 진단 안 함 + 불일치 목록 기록.
- **결과 타입 정밀 추론** (선행 spec 1.1.2 초안을 실측으로 보정): `정수 op 정수 → 정수`, `정수/실수 혼합 → 실수`, 비교/논리 → `논리`, `문자 + 문자 → 문자` 등. 실측과 다르면 실측 우선. 모호하면 Any.
...
- **사전 검증 (B-1 Task 0):** evaluator가 if-블록에 새 Environment를 만드는지 실측. 만약 if-블록 선언이 바깥으로 누수되는 시맨틱이면, narrowing용 pushScope가 "분기 내 선언 → 분기 밖 사용" 패턴에 TC006 오탐을 만든다 → 이 경우 스코프 대신 **narrow 오버레이 맵**(선언은 통과시키고 좁힘만 기록)으로 구현을 전환한다. 구현 방식 선택은 실측 결과에 따른다.
...
## D4. foreach/forrange 원소·범위 검사 (plan B-1, TC3xx 선사용)

D8의 TC3xx(컬렉션/원소)를 Phase B에서 2개 선사용한다. (카테고리 자체는 Phase E 소유)

| 코드 | 검사 |
|---|---|
| TC301 | forrange의 start/end가 정수 아님 (실측 기준 런타임 거부 조합만) |
| TC302 | foreach의 iterable이 순회 불가 타입 (배열/문자/사전/Any/Never 외 — 실측 기준) |

- foreach 원소 타입 검사: **문자 iterable → 원소는 문자**만 검사 가능 (elementType이 문자가 아니면 진단 — 실측에서 런타임도 거부할 때만). 배열은 평면 모델이라 원소 타입 검사 불가 → elementType 그대로 신뢰 (Phase E).
- forrange varType: 실측에서 정수 외가 런타임 거부되면 동일 원칙으로 진단.

**게이트:** 기존 for_each/forrange 골든 0건, `반복 정수 i = "a" 부터 10 까지:` 류 진단 (실측이 거부 확인 시).
...
- `자기.x = a` (생성자/메서드 내): 필드 타입 vs 값 타입 → TC002 재사용. 미지 필드면 TC201.
  - **주의 — 실측 필요 (B-2 Task 0):** 생성자에서 `자기.새필드 = ...`로 선언 안 된 필드를 동적으로 만드는 게 런타임에서 허용되는지. 허용이면 TC201은 **메서드에서만** 발화하고 생성자는 등록으로 처리.
...
- `부모` 키워드: 부모 클래스의 InstanceType으로 처리. 부모 없는 클래스에서 사용 시 처리 방식은 실측 후 결정.
- 메서드 내 재귀·상호 참조: 클래스의 필드/메서드 시그니처를 **본문 검사 전에 전부 등록**한 후 본문들을 검사한다 (클래스 내부는 hoisting 시맨틱 — evaluator가 클래스 전체를 한 번에 등록하므로 안전망 원칙과 합치. B-2 Task 0에서 재확인).
...
## 부록 A. 이항 연산자 실측 매트릭스 (B-1 Task 0에서 채움)

> 실측 전. plan B-1 Task 0 완료 시 본 섹션을 갱신한다. evaluator/VM 각각 기록하고, 불일치 조합은 ⚠️ 마크 + 런타임 일관성 이슈 목록에 복사한다.

## 부록 B. 런타임 일관성 이슈 목록 (발견 시 추가)

1. `실수 b = 1` — tree-walker 거부 / VM 통과 (Phase A Task 0 발견, 선행 spec 1.1)
