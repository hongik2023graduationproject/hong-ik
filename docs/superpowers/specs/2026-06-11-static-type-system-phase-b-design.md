...
> **✅ #1~#6 해소됨 (2026-06-12, `2026-06-12-runtime-consistency-design.md` 구현 완료)** — 이항 784조합·선언 42조합 재검증 불일치 0건.
> **#7 (2026-06-12 재특성화):** 제너레이터 표현 차이 — VM은 호출 시 **즉시 평가해 배열 반환** (`출력(g())` → `[1, 2, 3]`), evaluator는 **lazy GeneratorObject** (`출력(g())` 에러). foreach 순회는 양쪽 정상 동작 (초기 "침묵 0회" 기록은 오진). 실사용 차이는 무한 제너레이터(VM은 실행 제한까지 평가)·부수효과 타이밍·비순회 사용처. 심각도 낮음 — Phase B-3(제너레이터 흐름)에서 lazy 통일 여부 결정.
