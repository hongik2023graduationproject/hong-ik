...
TEST_F(VMTest, IfStatement) {
    string output = runVMWithOutput("정수 x = 10\n"
                                    "만약 x > 5 라면:\n"
                                    "    출력(\"크다\")\n"
                                    "\n");
...
TEST_F(VMTest, IfElseStatement) {
    string output = runVMWithOutput("정수 x = 3\n"
                                    "만약 x > 5 라면:\n"
                                    "    출력(\"크다\")\n"
                                    "아니면:\n"
                                    "    출력(\"작다\")\n"
...
TEST_F(VMTest, FunctionCall) {
    auto result = runVM("함수 더하기(정수 a, 정수 b) -> 정수:\n"
                        "    리턴 a + b\n"
                        "\n"
                        "더하기(3, 7)\n");
    auto* i     = dynamic_cast<Integer*>(result.get());
...
TEST_F(VMTest, MatchStatement) {
    string output = runVMWithOutput("정수 x = 2\n"
                                    "비교 x:\n"
                                    "    경우 1:\n"
                                    "        출력(\"일\")\n"
                                    "    경우 2:\n"
...
TEST_F(VMTest, MatchTypePattern) {
    string output = runVMWithOutput("정수 x = 42\n"
                                    "비교 x:\n"
                                    "    경우 문자:\n"
                                    "        출력(\"문자열\")\n"
                                    "    경우 정수:\n"
...
TEST_F(VMTest, MatchGuardCondition) {
    string output = runVMWithOutput("정수 x = 7\n"
                                    "비교 x:\n"
                                    "    경우 7 만약 x > 5:\n"
                                    "        출력(\"큰7\")\n"
                                    "    경우 7:\n"
...
TEST_F(VMTest, TryCatch) {
    string output = runVMWithOutput("시도:\n"
                                    "    정수 x = 10 / 0\n"
                                    "실패 오류:\n"
                                    "    출력(오류)\n"
                                    "\n");
...
TEST_F(VMTest, ClassFieldAccess) {
    string output = runVMWithOutput("클래스 동물:\n"
                                    "    문자 이름\n"
                                    "    생성(문자 이름):\n"
                                    "        자기.이름 = 이름\n"
                                    "\n"
...
TEST_F(VMTest, StringIterationUTF8) {
    auto output = runVMWithOutput("각각 문자 c \"안녕\" 에서:\n"
                                  "    출력(c)\n");
    EXPECT_EQ(output, "안\n녕\n");
}
...
TEST_F(VMTest, DefaultParameter) {
    auto result = runVM("함수 인사(문자 이름, 문자 접미사 = \"님\") -> 문자:\n"
                        "    리턴 이름 + 접미사\n"
                        "인사(\"홍길동\")\n");
    auto* s     = dynamic_cast<String*>(result.get());
    ASSERT_NE(s, nullptr);
...
TEST_F(VMTest, DefaultParameterAllDefaults) {
    auto result = runVM("함수 더하기(정수 a = 1, 정수 b = 2) -> 정수:\n"
                        "    리턴 a + b\n"
                        "더하기()\n");
    auto* i     = dynamic_cast<Integer*>(result.get());
    ASSERT_NE(i, nullptr);
...
TEST_F(VMTest, IndexAssignmentHashMap) {
    auto result = runVM("사전 정보 = {\"이름\": \"홍길동\"}\n"
                        "정보[\"나이\"] = \"25\"\n"
                        "정보[\"나이\"]\n");
    auto* s     = dynamic_cast<String*>(result.get());
    ASSERT_NE(s, nullptr);
...
TEST_F(VMTest, ImportBasic) {
    ...
    auto result = runVM("가져오기 \"" + tmpFile
                        + "\"\n"
                          "제곱(5)\n");
    auto* i     = dynamic_cast<Integer*>(result.get());
    ASSERT_NE(i, nullptr);
...
TEST_F(VMTest, ImportRejectsParentPath) {
    EXPECT_THROW({ runVM("가져오기 \"../secret.hik\"\n"); }, RuntimeException);
}

TEST_F(VMTest, ImportRejectsAbsoluteUnixPath) {
    EXPECT_THROW({ runVM("가져오기 \"/etc/passwd\"\n"); }, RuntimeException);
}

TEST_F(VMTest, ImportRejectsAbsoluteBackslashPath) {
    EXPECT_THROW({ runVM("가져오기 \"\\\\windows\\\\path\"\n"); }, RuntimeException);
}

TEST_F(VMTest, ImportRejectsWindowsDriveLetter) {
    EXPECT_THROW({ runVM("가져오기 \"C:/Windows/System32/cmd.exe\"\n"); }, RuntimeException);
}

// ===== 제너레이터/생산 =====

TEST_F(VMTest, GeneratorBasic) {
    auto output = runVMWithOutput("함수 세개() -> 정수:\n"
                                  "    생산 1\n"
                                  "    생산 2\n"
                                  "    생산 3\n"
                                  "각각 정수 값 세개() 에서:\n"
...
TEST_F(VMTest, GeneratorWithLoop) {
    auto output = runVMWithOutput("함수 범위(정수 n) -> 정수:\n"
                                  "    반복 정수 i = 0 부터 n 까지:\n"
                                  "        생산 i\n"
                                  "각각 정수 x 범위(3) 에서:\n"
                                  "    출력(x)\n");
...
TEST_F(VMTest, GeneratorEmpty) {
    auto output = runVMWithOutput("함수 빈것() -> 정수:\n"
                                  "    만약 false 라면:\n"
                                  "        생산 1\n"
                                  "각각 정수 x 빈것() 에서:\n"
                                  "    출력(x)\n");
...
TEST_F(VMTest, IntegrationDefaultsAndIndexAssignment) {
    auto output = runVMWithOutput("함수 초기화(정수 크기 = 3) -> 배열:\n"
                                  "    배열 결과 = [0, 0, 0]\n"
                                  "    반복 정수 i = 0 부터 크기 까지:\n"
                                  "        결과[i] = i * 2\n"
                                  "    리턴 결과\n"
...
TEST_F(VMTest, IntegrationGeneratorWithDefaults) {
    auto output = runVMWithOutput("함수 짝수(정수 최대 = 6) -> 정수:\n"
                                  "    반복 정수 i = 0 부터 최대 까지:\n"
                                  "        만약 i % 2 == 0 라면:\n"
                                  "            생산 i\n"
                                  "각각 정수 n 짝수() 에서:\n"
...
TEST_F(VMTest, FunctionCorrectTypes) {
    auto result = runVM("함수 합계(정수 x, 정수 y) -> 정수:\n"
                        "    리턴 x + y\n"
                        "\n"
                        "합계(3, 4)\n");
    auto* i     = dynamic_cast<Integer*>(result.get());
...
TEST_F(VMTest, OutputRoutesThroughIOContext) {
    ...
    EXPECT_NE(captured.find("안녕"), std::string::npos)
        << "VM output should route through IOContext.print, but captured: '" << captured << "'";
}
